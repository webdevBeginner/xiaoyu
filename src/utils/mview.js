...
Vue.filter("string23", function (value) {   //全局方法 Vue.filter() 注册一个自定义过滤器,必须放在Vue实例化前面
    value = value.substring(0, 23);
    return value;
});

Vue.filter("string19", function (value) {   //全局方法 Vue.filter() 注册一个自定义过滤器,必须放在Vue实例化前面
    value = value.substring(0, 19);
    return value;
});

Vue.filter("string10", function (value) {   //全局方法 Vue.filter() 注册一个自定义过滤器,必须放在Vue实例化前面
...
Vue.filter("laststring8", function (value) {   //全局方法 Vue.filter() 注册一个自定义过滤器,必须放在Vue实例化前面
            ...
            mview.socket.listen.onmessage = function (evt) {
                ...
                switch (json.Data.Code) {
                    ...
                    case '4014':
                        ...
                        break;
                    case '4015':
                        json.Data.Message = '本次提现USDT产生的手续费大于BCG可用余额，请重新输入';
                        break;
                    case '4016':
                ...
                if (pack != null && pack != undefined) {
                    //如果有状态码 且 是系统状态码
                    if (json.Data && (json.Data.Code == -30001 || json.Data.Code == -30002 || json.Data.Code == -30003 || json.Data.Code == -30099)) {
                        if (pack.error) { pack.error(json.Data.Message, json.IDentifier); }
                        return;
                    }
                    else {
